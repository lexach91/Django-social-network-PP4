...
class TestViews(TestCase):
    ...
    def test_send_friend_request(self):
        ...
        response = self.client.post(
            ...
            {'profile_id': self.user2.profile.id},
            HTTP_X_REQUESTED_WITH='XMLHttpRequest'
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(FriendRequest.objects.count(), 1)
        self.assertJSONEqual(
            response.content.decode('utf-8'), {'status': 'ok'})
        self.assertEqual(
            FriendRequest.objects.first().from_profile, self.user1.profile)
        self.assertEqual(
            FriendRequest.objects.first().to_profile, self.user2.profile)
    ...
    def test_accept_friend_request(self):
        ...
        FriendRequest.objects.create(
            from_profile=self.user2.profile, to_profile=self.user1.profile)
        response = self.client.post(
            self.accept_friend_request_url,
            {'profile_id': self.user2.profile.id},
            HTTP_X_REQUESTED_WITH='XMLHttpRequest'
        )
        self.assertEqual(response.status_code, 200)
        ...
        self.assertEqual(self.user1.profile.friends.first(),
                         self.user2.profile)
        self.assertEqual(self.user2.profile.friends.first(),
                         self.user1.profile)
    ...
    def test_decline_friend_request(self):
        ...
        FriendRequest.objects.create(
            from_profile=self.user2.profile, to_profile=self.user1.profile)
        response = self.client.post(
            self.decline_friend_request_url,
            {'profile_id': self.user2.profile.id},
            HTTP_X_REQUESTED_WITH='XMLHttpRequest'
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_cancel_friend_request(self):
        ...
        # create friend request
        FriendRequest.objects.create(
            from_profile=self.user1.profile, to_profile=self.user2.profile)
        FriendRequestEvent.objects.create(
            initiator=self.user1, target=self.user2)
        response = self.client.post(
            self.cancel_friend_request_url,
            {'profile_id': self.user2.profile.id},
            HTTP_X_REQUESTED_WITH='XMLHttpRequest'
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_my_friends_view(self):
        ...
        FriendRequest.objects.create(
            from_profile=self.user3.profile, to_profile=self.user1.profile)
