...
class TestModels(TestCase):
    ...
    def test_chat_str(self):
        ...
        self.assertEqual(
            str(chat),
            'Chat between {}'.format(
                ' and '.join([member.username for member in chat.members.all()])
            )
        )
    ...
    def test_message_str(self):
        ...
        self.assertEqual(
            str(message),           
            str(message.content).replace('<br>', ' ')
        )
    ...
    def test_message_sent_at(self):
        ...
        message.created_at = message.created_at - datetime.timedelta(minutes=60)
        self.assertEqual(
            message.sent_at,
            'an hour ago'
        )
        message.created_at = message.created_at - datetime.timedelta(minutes=120)
        self.assertEqual(
            message.sent_at,
            '3 hours ago'
        )
        ...
        message.created_at = message.created_at - datetime.timedelta(days=365*2)
        self.assertEqual(
            message.sent_at,
            '3 years ago'
        )
